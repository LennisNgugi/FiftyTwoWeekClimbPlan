...
Create a `.env` file in the file path `FiftyTwoWeekClimbPlan\.env` the following environment variables are required in file:
...
The application expects `.env` to be in the file path `net10.0` so do not delete once generated or error handler will appear when running the application:

```text
bin\Debug\net10.0\.env
```
The application expects `bot-trades.json` to be in the file path `net10.0` so do not delete once generated or error handler will appear when running the application:

```text
bin\Debug\net10.0\bot-trades.json
```
The application expects `stop-loss-cooldown.json` to be in the file path `net10.0` so do not delete once generated or error handler will appear when running the application:

```text
bin\Debug\net10.0\stop-loss-cooldown.json
```
The application expects `instruments.json` to be in the file path `net10.0` so do not delete once generated or error handler will appear when running the application:
...
Contains the Trading 212 instrument list used to match Clerusync Investment tickers with Trading 212 tickers. 
Due to copyright laws, I can not publish Trading212 instruments list used for mapping short name and trading212 tickers from a json file. However, the list can be accessed with the python script `trading212instruments.py`.
...
Renewable solar energy: Add bash script that automates running on raspberry pi. 
